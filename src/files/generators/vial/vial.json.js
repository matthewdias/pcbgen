...
class VialJSON extends Generator {
	...
	fillTemplate() {
        ...
        const labels = Object.keys(keyboard.layoutOptions)
            .map(layout => [layout].concat(Object.keys(keyboard.layoutOptions[layout])))

        keyboard.keys.map((key, index) => {
            let keyState = {};

            if (index === 0) {
                state = Object.assign({}, key.state)
                keyState = Object.assign({}, key.state)
            } else {
                for (const mod in key.state) {
                    if (ignored_states.includes(mod)) {
                        continue;
                    }

                    if (key.state[mod] != state[mod] || mod == 'xOffset' || mod == 'yOffset') {
                        if (mod == 'x' || mod == 'y') {
                            continue;
                        }
            ...
            const optionLegend = (key.layout && key.layoutOption)
                ? `\n\n\n${layouts.indexOf(key.layout)},${labels[layouts.indexOf(key.layout)].indexOf(key.layoutOption) - 1}` : ''
