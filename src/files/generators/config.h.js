...
class ConfigH extends Generator {
	...
	fillTemplate() {
		...
		return {
			...
			'led_num': keyboard.pins.num ? '#define LED_NUM_LOCK_PIN ' + keyboard.pins.num : '',
			'led_caps': keyboard.pins.caps ? '#define LED_CAPS_LOCK_PIN ' + keyboard.pins.caps : '',
			'led_scroll': keyboard.pins.scroll ? '#define LED_SCROLL_LOCK_PIN ' + keyboard.pins.scroll : '',
			'led_compose': keyboard.pins.compose ? '#define LED_COMPOSE_PIN ' + keyboard.pins.compose : '',
			'led_kana': keyboard.pins.kana ? '#define LED_KANA_PIN ' + keyboard.pins.kana : ''
		};
	}
