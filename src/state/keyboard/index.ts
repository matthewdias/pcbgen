...
class Keyboard {
   ...
   */
  constructor(state: unknown, json?: unknown) {
    this.state = state
    this.keys = []
    this.layouts = {}
    this.layoutOptions = {}
    ...
    this.settings = {
      ...
      deviceVer: '0001',
      manufacturer: '',
      connectorType: C.CONNECTOR_TYPE_C,
  ...
  importKLE(json) {
    ...
    for (const row of json) {
      ...
      for (const entry of row) {
        ...
        const key = new Key(this, keyIndex++, entry, { rowIndex, ...state })
        this.keys.push(key)

        // Update bounds.
        this.bounds.min.x = Math.min(
          this.bounds.min.x,
          key.pos.x + key.bounds.min.x
        )
        this.bounds.max.x = Math.max(
          this.bounds.max.x,
          key.pos.x + key.bounds.max.x
        )
        this.bounds.min.y = Math.min(
          this.bounds.min.y,
          key.pos.y + key.bounds.min.y
        )
        this.bounds.max.y = Math.max(
          this.bounds.max.y,
          key.pos.y + key.bounds.max.y
        )
    ...
    for (let i = 0; i < this.rows; i += 1) {
      usedPins += 1
      this.pins.row.push(
        C.PINS[this.controller][
          Math.min(usedPins, C.PINS[this.controller].length)
        ]
      )
    }
  ...
  updateWiring() {
    ...
    for (const key of this.keys) {
      // Update invalid keys.
      if (key.row >= this.rows) {
        key._row = this.rows - 1
      }
      if (key.col >= this.cols) {
        key._col = this.cols - 1
      }
    ...
    Object.keys(this.wiring).forEach((position) => {
      ...
      keys.forEach((key, index) => {
        ...
        key.matrixPos = `${key.rowHex}${key.colHex}${
          keys.length > 1 ? `_${index}` : ''
        }`
      })
    })
  ...
  estimateWiring() {
    ...
    for (const key of this.keys) {
      ...
      const row = Math.floor(key.center.y)
      const col = Math.floor(key.center.x)
  ...
  updatePins() {
    ...
    // Remove invalid pins.
    for (let i = 0; i < this.pins.row.length; i += 1) {
      if (!allowedPins.includes(this.pins.row[i])) {
        this.pins.row[i] = defaultPin
      }
    }
    for (let i = 0; i < this.pins.col.length; i += 1) {
      if (!allowedPins.includes(this.pins.col[i])) {
        this.pins.col[i] = defaultPin
      }
    }
    ...
    while (this.pins.row.length < this.rows) {
      this.pins.row.push(defaultPin)
    }
    while (this.pins.col.length < this.cols) {
      this.pins.col.push(defaultPin)
    }
  ...
  verify() {
    ...
    if (this.pins.num && usedPins.includes(this.pins.num)) {
      this.valid = overlappingPins = false
    }
    usedPins.push(this.pins.num)
    if (this.pins.caps && usedPins.includes(this.pins.caps)) {
      this.valid = overlappingPins = false
    }
    usedPins.push(this.pins.caps)
    if (this.pins.scroll && usedPins.includes(this.pins.scroll)) {
      this.valid = overlappingPins = false
    }
    usedPins.push(this.pins.scroll)
    if (this.pins.compose && usedPins.includes(this.pins.compose)) {
      this.valid = overlappingPins = false
    }
    usedPins.push(this.pins.compose)
    if (this.pins.kana && usedPins.includes(this.pins.kana)) {
      this.valid = overlappingPins = false
    }
    usedPins.push(this.pins.kana)
    if (this.pins.led && usedPins.includes(this.pins.led)) {
      this.valid = overlappingPins = false
    }
    usedPins.push(this.pins.led)
    if (this.pins.rgb && usedPins.includes(this.pins.rgb)) {
      this.valid = overlappingPins = false
    }
    ...
    if (missing.length) {
      this.warnings.push(
        `Your keymap is missing the key${
          missing.length > 1 ? 's' : ''
        }: ${missing.join(', ')}.`
      )
    }
    ...
    if (!inKeymap.has('RESET')) {
      this.warnings.push('Your keymap is missing a soft reset key.')
    }
  }
  ...
  static deserialize(state, serialized) {
    ...
    const { controller } = serialized
    const { bounds } = serialized
    const { rows } = serialized
    const { cols } = serialized
    const { pins } = serialized
    const { macros } = serialized
    const { quantum } = serialized
...
type IKeyboard = typeof Keyboard
